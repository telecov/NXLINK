...
/* =============================
   CARGA CONFIG PRINCIPAL (para logo / textos)
   ============================= */
...
$ip_eth = trim((string)@shell_exec("nmcli -t -f IP4.ADDRESS device show " . escapeshellarg($ETH_IF) . " 2>/dev/null | head -n 1 | cut -d':' -f2"));
$gw_eth = trim((string)@shell_exec("nmcli -t -f IP4.GATEWAY device show " . escapeshellarg($ETH_IF) . " 2>/dev/null | head -n 1 | cut -d':' -f2"));
    ...
    <div class="row align-items-center g-3">
      ...
      <div class="col-md-8 col-12">
        <div class="header-title text-light d-flex align-items-center gap-3">
          <img 
  		src="<?php echo htmlspecialchars($logoConfiguracion); ?>" 
...
<div class="row g-4">
  ...
  <div class="col-lg-6">
    <div class="card-custom">
      ...
      <p class="small-label"><?php echo __("cfg_status"); ?>:
        <strong><?php echo htmlspecialchars(strtoupper($serviceStatus)); ?></strong>
      </p>

      <p class="small-label"><?php echo __("cfg_server_ip"); ?>:<br><?php echo htmlspecialchars($ipServidor); ?></p>
